...
// ép ranh giới hexagon — package domain của accounting phải THUẦN, KHÔNG import
// hạ tầng (pgx, huma, net/http, sqlc appdb, các platform adapter, app, adapters).
// Cho phép common/money + shopspring/decimal (shared kernel trung lập domain —
// thư viện số thập phân, KHÔNG phải hạ tầng). Dùng go/build (stdlib).
func TestDomain_NoInfraImports(t *testing.T) {
	...
	forbidden := []string{
		...
		"github.com/Maneva-AI/namviet-backend/internal/accounting/app",      // không phụ thuộc ngược lên app
		"github.com/Maneva-AI/namviet-backend/internal/accounting/internal", // không phụ thuộc adapter
	}
