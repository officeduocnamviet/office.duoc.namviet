...
// ép ranh giới hexagon — package domain của purchasing phải THUẦN, KHÔNG import hạ
// tầng (pgx, huma, net/http, sqlc appdb, các platform adapter, app, adapters). Cho
// phép common/money + shopspring/decimal (shared kernel trung lập domain). Dùng
// go/build (stdlib).
func TestDomain_NoInfraImports(t *testing.T) {
	...
	forbidden := []string{
		...
		"github.com/Maneva-AI/namviet-backend/internal/purchasing/app",      // không phụ thuộc ngược lên app
		"github.com/Maneva-AI/namviet-backend/internal/purchasing/internal", // không phụ thuộc adapter
		"github.com/Maneva-AI/namviet-backend/internal/inventory",           // không kéo bounded context khác vào domain
